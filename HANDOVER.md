...
## S124 完了内容（2026-04-08）

### 機能追加（1件）

| 内容 | 詳細 |
|---|---|
| IDLE-EXP（修行モード）実装 | `gs.player.idleTraining` 状態管理を DEFAULT_STATE・マイグレーションに追加。`startIdleTraining` / `stopIdleTraining` / `checkIdleTraining` 関数を追加。5分サイクルで `gainExp(10 + floor * 5)`。10サイクルで疲弊付与。`worldRank>=1`・非ダンジョン中・非疲弊時に解禁。ショップ SHOP_SECONDARY に「🧘 修行」ボタンを追加 |

---

## S123 完了内容（2026-04-08）

### 機能追加（1件）・設計（1件）・確認（1件）・ドキュメント（1件）

| 内容 | 詳細 |
|---|---|
| COLLECTION-REWARD表示 | `openCollectionModal()` の `renderHeader()` にフルコンプ達成時「🏆 コンプリート済」バッジを追加。プログレスバーを達成時に金色（legendary-bright）に変更 |
| IDLE-TRAINING バランス設計 | balance.md §23 に IDLE-EXP（修行モード）/ IDLE-GOLD（交易モード）の数値設計・実装メモを先行追記 |
| BUG-HUNT-S122 | `retroCheckCollection` チェーンで SP 二重取得しないことを確認。`collectionFullCompleted=true` で early return → 問題なし |
| arch.md S122〜S123更新 | SP ボーナス・コレクションバッジのアーキテクチャを追記 |

---

## S122 完了内容（2026-04-08）

### 機能追加（1件）・確認（1件）・ドキュメント（2件）

| 内容 | 詳細 |
|---|---|
| COLLECTION-REWARD拡充 | フルコンプ達成時に SP+3 を付与。`collectionFullCompleted=true` の直後に `gs.player.skillPoints += 3`。冒険ログに「SP+3獲得」を追記。モーダルに「✨ ボーナス SP+3 獲得！」を表示。真エンド解禁後の再挑戦時も再付与される |
| BUG-HUNT-S121 | `orderSuccessSerif` は注文品購入ブロックのみで使用。通常購入ログ（regularSerif）には混入しないことを確認 |
| spec.md 常連セリフ仕様 | §6-4 にセリフシステムテーブル（6キー×種数）を追記 |
| spec.md テスト手順 | S119 sub/cloak/seal テストケースを手順6に追記済み（前セッション）|
...
| 内容 | 詳細 |
|---|---|
| 注文成功セリフ6種追加 | `REGULAR_SERIFS.orderSuccess`（6種）を追加。`checkRegularVisits()` の注文品購入ログで固定文字「注文の品、確かに！」→ランダムセリフに変更 |
| BUG-HUNT-S120 | `%s` は全セリフで1箇所のみ → `String.replace()` で問題なし確認 |
| issues.md 最終同期 | S120〜S121 の常連注文セリフ拡充を issues.md に追記 |
...
| 内容 | 詳細 |
|---|---|
| REGULAR-DEEPENING: 注文セリフ | `REGULAR_SERIFS.order`（5種）を追加。`checkRegularOrders()` の注文ログで `%s` を typeLabel に置換してランダム表示。既存の low/mid/high/gift セリフに影響なし |
| BUG-HUNT-S119 | `isOrderFilled` は `ITEM_MASTER.type` で直接比較するため sub/cloak/seal も正しく動作することを確認 |
| spec.md テスト手順更新 | S119 の sub/cloak/seal フィルタ確認テストを手順6に追記 |
