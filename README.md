...
### 直近の主な変更（S51）
- デイリーミッション：5種ランダム2種抽選化（新種別 sell_rare / sell_count 追加）
- 帰還リザルト「📦 まとめて出品する」ボタン追加
- 転生スキル「深淵の覇道」3種追加（worldRank50解禁）
- Analytics可視化モーダル追加（ステータスタブ「📊 分析」）
- バグ修正5件（LOGIC-3 / UX-2 / BUG-3 / LOGIC-1 / init側ガード漏れ）
