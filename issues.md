...
- **内容**: 注文発生・注文成功のショップログが固定文字列のみで単調だった
- **修正（S120）**: `REGULAR_SERIFS.order`（5種）を追加。注文発生時に `%s` → typeLabel で置換してランダム表示
- **修正（S121）**: `REGULAR_SERIFS.orderSuccess`（6種）を追加。注文成功時にランダムセリフを使用
- **影響範囲**: `checkRegularOrders()` と `checkRegularVisits()` の注文ログのみ

---

## ✅ 修正済み（S119）

### OPENLISTMODAL-TYPEORDER（S119）
- **内容**: `openListModal` の `typeOrder` に `sub`/`cloak`/`seal` が含まれておらず、常連注文タイプがこれらの場合 `filterType` フィルタが効かなかった
- **原因**: S109 で `filterType` 機能を追加した際、`typeOrder` が他モーダルと異なる不完全なリストのままだった
- **修正**: `typeOrder` に `sub`/`cloak`/`seal` を追加。`TYPE_ICON` に `cloak:"🧥"` を追加
- **影響範囲**: `openListModal` のタイプフィルタボタン表示のみ。出品ロジックに影響なし
